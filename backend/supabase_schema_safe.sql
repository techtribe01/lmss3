-- LMS Database Schema for Supabase PostgreSQL (Safe Version)
...
DROP POLICY IF EXISTS "Service role has full access to task_submissions" ON task_submissions;
DROP POLICY IF EXISTS "Service role has full access to mock_interviews" ON mock_interviews;
DROP POLICY IF EXISTS "Service role has full access to reports" ON reports;
DROP POLICY IF EXISTS "Service role has full access to fee_reminders" ON fee_reminders;
DROP POLICY IF EXISTS "Service role has full access to materials" ON materials;
DROP POLICY IF EXISTS "Service role has full access to certificates" ON certificates;
...
BEGIN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_courses_mentor_id') THEN
        CREATE INDEX idx_courses_mentor_id ON courses(mentor_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_courses_approval_status') THEN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_attendance_course_id') THEN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_tasks_course_id') THEN
        CREATE INDEX idx_tasks_course_id ON tasks(course_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_task_submissions_task_id') THEN
        CREATE INDEX idx_task_submissions_task_id ON task_submissions(task_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_task_submissions_student_id') THEN
        CREATE INDEX idx_task_submissions_student_id ON task_submissions(student_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_mock_interviews_student_id') THEN
        CREATE INDEX idx_mock_interviews_student_id ON mock_interviews(student_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_mock_interviews_mentor_id') THEN
        CREATE INDEX idx_mock_interviews_mentor_id ON mock_interviews(mentor_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_materials_course_id') THEN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_certificates_course_id') THEN
...
-- Create new policies for service role (bypass RLS for backend)
